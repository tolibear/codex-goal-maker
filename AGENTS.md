...
- `goal-prep/` is the canonical installable Goal Prep skill payload.
- `deep-intake/` is the companion deep alignment skill payload.
- `goalbuddy/` is a short compatibility alias, not the full skill payload.
- `goal-prep/agents/` holds Codex TOML agent definitions (`goal_*.toml`).
- `plugins/goalbuddy/agents/` holds Claude Code markdown subagents (`goal-*.md`).
- `goal-prep/scripts/` stays inside the skill because installed skill instructions call those scripts.
...
- `plugins/goalbuddy/` is the repo-local plugin scaffold. It contains:
  ...
  - `skills/goal-prep/` — shared Goal Prep skill payload.
  - `skills/deep-intake/` — companion Deep Intake skill payload.
  - `skills/goalbuddy/` — compatibility alias.
...
When improving this repo, consider README, `goal-prep/SKILL.md`, `deep-intake/SKILL.md`, generated `$goal-maker` compatibility behavior, templates, checker behavior, CLI UX, plugin UX (both Codex and Claude Code), examples, package contents, extension catalog shape, and `extend/` documentation. Do not assume a request only touches code.

## Package Rules

- Keep the runtime dependency-free unless there is a strong reason.
- Keep `goal-prep/` installable as the canonical skill directory.
