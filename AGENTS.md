...
- `goal-prep/` is the canonical installable skill payload (used by both Codex and Claude Code as `$goal-prep` / `/goal-prep`).
- `goalbuddy/` is only the legacy compatibility alias; keep it small and do not put canonical workflow files there.
...
- `plugins/goalbuddy/` is the repo-local plugin scaffold. It contains:
  ...
  - `skills/goal-prep/` — canonical shared skill payload.
  - `skills/goalbuddy/` — legacy compatibility alias.
...
When improving this repo, consider README, `goal-prep/SKILL.md`, generated `$goal-maker` compatibility behavior, templates, checker behavior, CLI UX, plugin UX (both Codex and Claude Code), examples, package contents, extension catalog shape, and `extend/` documentation. Do not assume a request only touches code.
